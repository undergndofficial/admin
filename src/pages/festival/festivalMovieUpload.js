...
function FestivalMovieUpload() {
  ...
  const upload = () => {
    ...
    axios.put(apiUrl + '/festival', movieInfo_1).then(async (res) => {
      const mov_seq = res.data.rs.mov_seq;
      console.log(mov_seq);
      const formData = new FormData();
      formData.append('movfile', movieInfo_2.mov_file);
      await axios.put(`${apiUrl}/festival/${mov_seq}/movfile`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      await axios.post(
        apiUrl + '/festival/' + mov_seq + '/videoinfo',
        movieInfo_3
      );
      await axios.post(
        apiUrl + '/festival/' + mov_seq + '/remark',
        movieInfo_4
      );
      await axios
  ...
  const uploadInputs = {
    ...
    langage: {
      ...
      isInput: false,
      type: 'select',
      addDataName: 'lang_code',
      selectMenus: langages,
      isForeign: true,
      valueKey: 'lang_name',
      seq: 'lang_code',
    },
    ...
    coloration: {
      ...
      isInput: false,
      type: 'select',
      addDataName: 'coloration',
      selectMenus: colorations,
      isForeign: true,
      valueKey: 'coloration_txt',
      seq: 'coloration',
    },
    remark: {
      name: '문의/요청',
  ...
  useEffect(() => {
    axios
      .get(apiUrl + '/common/category')
      .then((res) => setCategorys(res.data.rs));

    axios.get(apiUrl + '/common/gerne').then((res) => setGenres(res.data.rs));

    axios
      .get(apiUrl + '/common/rating-us')
      .then((res) => setRatings(res.data.rs));

    axios.get(apiUrl + '/common/lang').then((res) => setLangages(res.data.rs));

    axios
      .get(apiUrl + '/common/nationality')
      .then((res) => setNations(res.data.rs));

    axios
      .get(apiUrl + '/common/coloration')
    ...
    <div className="festivalUpload">
      ...
      colorations[0] ? (
        <Upload
          ...
          addedUploadDatas={addedUploadDatas}
          setAddedUploadDatas={setAddedUploadDatas}
          upload={upload}
        />
      ) : (
        <div>Loading...</div>
